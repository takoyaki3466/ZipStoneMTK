...
public class LangENProvider extends LanguageProvider {
    public LangENProvider(PackOutput output, String modid) {
        super(output, modid, "en_us");
    }

    @Override
    protected void addTranslations() {
        add(BlocksInit.BLOCKS_AND_ITEMS.block(), "cobblestone");
    }

    private void add(List<RegistrySupplier<Block>> blocks, String baseName) {
        for (int i = 0; i < blocks.size(); i++) {
            BigInteger zip = BigInteger.valueOf(9).pow(i + 1);
            add(blocks.get(i).get(), zip + "x compressed " + baseName);
        }
    }
}
